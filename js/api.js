        ...
        if (data.status === 'success' && data.projects) {
          ...
          var formattedProjects = data.projects.map(function(row) {
            return {
              ...
              category: row['類別'] || row['專案類別'] || row.category || (window.Calculator ? window.Calculator.getProjectCategory(row) : ((row['是否為智慧建築'] === '是' || row.isSmartBuilding === true) ? '智慧建築' : '一般建築')),
              isSmartBuilding: (row['是否為智慧建築'] === '是' || row.isSmartBuilding === true || (row['類別'] || row['專案類別']) === '智慧建築'),
              smartGrade: row['智慧建築等級'] || row.smartGrade || '無',
              smartMultiplier: (row['智慧建築倍率'] !== undefined && row['智慧建築倍率'] !== '') ? parseFloat(row['智慧建築倍率']) : ((row['倍率'] !== undefined && row['倍率'] !== '') ? parseFloat(row['倍率']) : (row.smartMultiplier !== undefined ? parseFloat(row.smartMultiplier) : (window.Calculator && window.Calculator.SMART_GRADES[row['智慧建築等級'] || row.smartGrade] ? window.Calculator.SMART_GRADES[row['智慧建築等級'] || row.smartGrade] : 1.0))),
              modScore: (row['修改點數'] !== undefined && row['修改點數'] !== '') ? parseFloat(row['修改點數']) : (row.modScore !== undefined ? parseFloat(row.modScore) : null),
